...
func TestProcessor_Process_CloneError(t *testing.T) {
    p := NewProcessor()
    repo := domain.GitRepository{Url: "https://example.com/repo.git"}
    pp := domain.ProviderConfig{}
    cfg := domain.Config{}

    withStubbedFns(t, func() {
        cloneGitFn = func(repo domain.GitRepository, config domain.ProviderConfig) (*gogit.Repository, billy.Filesystem, error) {
            return nil, nil, errors.New("boom")
        }
        // Not expected to be called
        isValidForBoneCloneFn = func(repo *gogit.Repository, config domain.Config) (bool, error) { return false, nil }
        copyFilesFn = func(repo *gogit.Repository, fs billy.Filesystem, cfg domain.FileConfig, pp domain.ProviderConfig) error { return nil }
...
func TestProcessor_Process_ValidateError(t *testing.T) {
    ...
    withStubbedFns(t, func() {
        ...
        isValidForBoneCloneFn = func(repo *gogit.Repository, config domain.Config) (bool, error) {
...
func TestProcessor_Process_NotValid_SkipsCopy(t *testing.T) {
    ...
    withStubbedFns(t, func() {
        ...
        isValidForBoneCloneFn = func(repo *gogit.Repository, config domain.Config) (bool, error) { return false, nil }
        copyCalled := false
        copyFilesFn = func(repo *gogit.Repository, fs billy.Filesystem, cfg domain.FileConfig, pp domain.ProviderConfig) error {
            copyCalled = true
            return nil
        }
...
func TestProcessor_Process_Valid_CopySuccess(t *testing.T) {
    ...
    withStubbedFns(t, func() {
        ...
        isValidForBoneCloneFn = func(repo *gogit.Repository, config domain.Config) (bool, error) { return true, nil }
        copyCalled := false
        copyFilesFn = func(repo *gogit.Repository, fs billy.Filesystem, cfg domain.FileConfig, pp domain.ProviderConfig) error {
            copyCalled = true
            return nil
