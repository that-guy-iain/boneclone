...
func TestNewProcessorForConfig_SelectsProcessors(t *testing.T) {
	p1 := domain.NewProcessorForConfig(domain.Config{Git: domain.GitConfig{PullRequest: false}}, nil, nil)
	if _, ok := p1.(*domain.Processor); !ok {
		t.Fatalf("expected *domain.Processor for PullRequest=false, got %T", p1)
	}

	p2 := domain.NewProcessorForConfig(domain.Config{Git: domain.GitConfig{PullRequest: true}}, nil, nil)
